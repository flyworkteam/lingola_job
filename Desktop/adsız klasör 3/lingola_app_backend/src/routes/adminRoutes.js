...
async function safeQuery(queryFn, fallback) {
  try {
    ...
    console.error("[admin dashboard query error]", err.message || err);
    return fallback;
  }
}
...
router.get("/", async (req, res) => {
  try {
    const [langCount, trackCount, wordCount, userCount, recentTracks, latestUsers] = await Promise.all([
      safeQuery(() => pool.query("SELECT COUNT(*) AS c FROM languages"), { rows: [{ c: 0 }] }),
      safeQuery(() => pool.query("SELECT COUNT(*) AS c FROM learning_tracks"), { rows: [{ c: 0 }] }),
      safeQuery(() => pool.query("SELECT COUNT(*) AS c FROM words"), { rows: [{ c: 0 }] }),
      safeQuery(() => pool.query("SELECT COUNT(*) AS c FROM users"), { rows: [{ c: 0 }] }),
      safeQuery(() => pool.query("SELECT id, title, language_code, created_at FROM learning_tracks ORDER BY created_at DESC LIMIT 5"), { rows: [] }),
      safeQuery(() => pool.query("SELECT id, first_name, last_name, email, created_at FROM users ORDER BY created_at DESC LIMIT 5"), { rows: [] }),
    ]);

    const langCountRows = langCount.rows || [{ c: 0 }];
    const trackCountRows = trackCount.rows || [{ c: 0 }];
    const wordCountRows = wordCount.rows || [{ c: 0 }];
    const userCountRows = userCount.rows || [{ c: 0 }];
    const recentTracksRows = recentTracks.rows || [];
    const latestUsersRows = latestUsers.rows || [];
    ...
    const tracksHtml = recentTracksRows.length
      ? recentTracksRows.map((t) => `<div class="list-row"><strong>${escapeHtml(t.title)}</strong><span class="muted">${escapeHtml(t.language_code)}</span><span class="muted">${new Date(t.created_at).toLocaleDateString("tr-TR")}</span></div>`).join("")
      : "<div class=\"list-row muted\">Henüz track yok.</div>";
    const usersHtml = latestUsersRows.length
      ? latestUsersRows.map((u) => {
    ...
    const html = layout(
      ...
      <div class="stat-row">
        <div class="stat-card blue">
          ...
          <div class="num">${Number(langCountRows[0]?.c ?? 0)}</div>
        ...
        <div class="stat-card orange">
          ...
          <div class="num">${Number(userCountRows[0]?.c ?? 0)}</div>
    ...
    const html = layout(
      "Hata",
      `
      <h2>Dashboard hatası</h2>
      <p class="muted">Veritabanı veya sunucu kaynaklı bir hata oluştu.</p>
      <div class="card" style="border-color:#fecaca; background:#fef2f2; color:#991b1b;">
        <strong>Hata:</strong> ${escapeHtml(message)}
      </div>
      <p><a href="/admin">Dashboard'a tekrar dene</a></p>
      `,
      "/admin"
    );
...
router.get("/users", async (req, res) => {
  try {
    const deleted = req.query.deleted ? String(req.query.deleted) : "";
    ...
    const flashHtml = deleted
      ? `<div class="card" style="border-color:#bbf7d0; background:#f0fdf4; color:#166534;">${escapeHtml(deleted)} silindi.</div>`
      : errorMessage
        ? `<div class="card" style="border-color:#fecaca; background:#fef2f2; color:#991b1b;">${escapeHtml(errorMessage)}</div>`
        : "";
      ...
      .map(
        (u) =>
          `<tr>
            ...
            <td>
              <form method="post" action="/admin/users/${u.id}/delete" onsubmit="return confirm('Bu kullanıcı ve bağlı veriler silinsin mi?');">
                <button type="submit" style="background:#dc2626;">Sil</button>
              </form>
            </td>
          </tr>`
      )
    ...
    const html = layout(
      ...
      `
      <h2>Users</h2>
      <p class="muted">Silme işlemi Firebase giriş hesabını ve bağlı uygulama verilerini kaldırır.</p>
      ${flashHtml}
      <table>
        <thead><tr><th>ID</th><th>Email</th><th>Name</th><th>Firebase UID</th><th>Created</th><th>Action</th></tr></thead>
        <tbody>${rowsHtml}</tbody>
      </table>
      `,
      "/admin/users"
    ...
    const message = err.message || String(err);
    const hint = err.code === "ETIMEDOUT" || message.includes("ETIMEDOUT")
      ? " Veritabanı sunucusuna ağdan ulaşılamıyor (VPN, firewall veya sunucu erişimini kontrol edin)."
      : "";
    const html = layout(
      "Hata",
      `
      <h2>Kullanıcılar yüklenemedi</h2>
      <p class="muted">Veritabanı bağlantı hatası.</p>
      <div class="card" style="border-color:#fecaca; background:#fef2f2; color:#991b1b;">
        <strong>Hata:</strong> ${escapeHtml(message)}${escapeHtml(hint)}
      </div>
      <p><a href="/admin/users">Tekrar dene</a> · <a href="/admin">Dashboard</a></p>
      `,
      "/admin/users"
    );
    res.status(500).send(html);
...
router.post("/users/:id/delete", async (req, res) => {
  ...
  if (Number.isNaN(userId) || userId < 1) {
    return res.redirect("/admin/users?error=" + encodeURIComponent("Geçersiz kullanıcı ID"));
  }

  try {
    const result = await pool.query(
      "SELECT id, email, firebase_uid FROM users WHERE id = $1 LIMIT 1",
      [userId]
    );

    if (result.rows.length === 0) {
      return res.redirect("/admin/users?error=" + encodeURIComponent("Kullanıcı bulunamadı"));
    }

    const user = result.rows[0];
    await deleteFirebaseAuthUser(user.firebase_uid);
    await pool.query("DELETE FROM users WHERE id = $1", [userId]);

    const deletedLabel = user.email || `Kullanıcı #${userId}`;
    return res.redirect("/admin/users?deleted=" + encodeURIComponent(deletedLabel));
  } catch (err) {
    console.error("Admin user delete error:", err);

    const message =
      err.code === "FIREBASE_ADMIN_NOT_CONFIGURED"
        ? "Firebase Admin yapılandırılmadığı için kullanıcı silinemedi"
